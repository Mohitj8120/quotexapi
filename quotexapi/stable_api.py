...
class Quotex:
    ...
    async def auto_trade(self, strategy, asset_name="XAUUSD_otc"):
        """ Automatically trade based on strategy """
        while True:
            end_from_time = time.time()
            offset = 3600  # 1 hour
            period = 60  # 1 minute
            candles = await self.get_candles(asset_name, end_from_time, offset, period)
            close_prices = [candle['close'] for candle in candles]
            if await strategy(asset_name, close_prices):
                await asyncio.sleep(60)  # Wait for the trade duration before checking again
